      ...
      <header className="bg-slate-800 border-b border-slate-700">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center space-x-4 lg:space-x-6 text-sm text-gray-300">
            ...
            <Link
              to="/login"
              className="italic font-semibold bg-teal-500 text-slate-900 px-4 py-2 rounded hover:bg-teal-400 transition"
            >
            ...
            {!token && (
              <Link
                to="/register"
                className="italic font-semibold bg-slate-200 text-slate-900 px-4 py-2 rounded hover:bg-teal-400 transition"
              >
                Register
              </Link>
            )}
        ...
        <motion.div
          ...
          className="overflow-hidden md:hidden bg-slate-800 border-t border-slate-700"
        >
          <div className="px-6 py-4 space-y-4 text-sm text-gray-300">
            <Link to="/features" className="block hover:text-teal-400" onClick={() => setIsOpen(false)}>Features</Link>
            <Link to="/aboutUs" className="block hover:text-teal-400" onClick={() => setIsOpen(false)}>About Us</Link>
            <Link to="/login" className="block hover:text-teal-400" onClick={() => setIsOpen(false)}>
              {token ? "Dashboard" : "Login"}
            </Link>
            {!token && (
              <Link
                to="/register"
                className="block bg-slate-200 text-slate-900 px-4 py-2 rounded hover:bg-teal-400 transition"
      ...
      <main className="flex flex-1 flex-col lg:flex-row items-center justify-between px-6 sm:px-10 py-10 sm:py-20 gap-8 sm:gap-10">
        ...
        >
          ...
          <div className="mt-6 flex flex-col sm:flex-row sm:space-x-4 space-y-3 sm:space-y-0">
            <Link
              to={"/register"}
              className="bg-teal-500 text-slate-900 px-6 py-3 rounded hover:bg-teal-400 transition"
            >
              Get Started
            </Link>
            <Link
              to={"/features"}
              className="bg-transparent text-teal-400 border border-teal-500 px-6 py-3 rounded hover:bg-slate-800 transition"
            >
              Watch Demo
            </Link>
          </div>
